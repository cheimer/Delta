...
ADeltaBaseCharacter::ADeltaBaseCharacter()
{
	...
	NiagaraPoolingComponent = CreateDefaultSubobject<UNiagaraPoolingComponent>(TEXT("NiagaraPoolingComponent"));
...
void ADeltaBaseCharacter::BeginSkill(const EDeltaSkillType SkillType)
{
	...
	USkillDataAsset* SkillData = FindSkillDataAsset(SkillType);
	if (!SkillData || SkillData->Skill.IsNull()) return;
...
{
	USkillDataAsset* SkillData = FindSkillDataAsset(SkillType);
	if (!SkillData || SkillData->AnimMontage.IsNull()) return 0.0f;
...
{
	UObjectPoolSubsystem* MeshPool = UObjectPoolSubsystem::Get(GetWorld());
	if (!MeshPool) return;
	...
	if (bIsVisible)
	{
		if (CachedSkillMesh)
		{
			...
			MeshPool->ReturnObject(CachedSkillMesh);
			CachedSkillMesh = nullptr;
		}
		
		CachedSkillMesh = MeshPool->GetObject(MeshClass);
...
void ADeltaBaseCharacter::LoadSkillAnim()
{
	...
	{
		if (SkillDataAsset->AnimMontage.IsNull()) continue;
