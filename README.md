...
- 💾 **완전한 옵션 시스템**: 그래픽, 사운드, 키 리매핑, 세이브/로드 등
- 🤖 **스마트 AI**: 상태 기반 스킬 선택, 페이즈 전환
- 👥 **동료 시스템**: 실시간 캐릭터 교체 및 팀 관리
- 🎬 **시네마틱 연출**: 포스트 프로세스, 타임 딜레이션, 컷신

## 🎬 데모 영상

<img width="50%" src="https://github.com/user-attachments/assets/e35a474a-89f0-4ae1-abdd-1e4ad9be4a4e" alt="ERROR! 하단 링크로 대체함"/>

▶️ https://youtu.be/6eYMrQpc2p8
...
| **아키텍처** | Subsystem Pattern, Object Pooling, Factory Patten |
...
UNiagaraPoolingComponent   // 나이아가라 액터 오브젝트 풀링
```
...
World Subsystems:
├── ObjectPoolSubsystem     // 오브젝트 풀링 액터 관리
...
**해결**:
- BehaviorTree, AnimNotifyState 가 아닌 해당 캐릭터에 저장
- BehaviorTree 에 저장해야 한다면 NodeMemory 사용

### 10. 성능 최적화
**문제**: 캐릭터를 많이 배치할 경우 간혈적인 프레임 드랍 발생

**분석**: 프로파일링 결과 여러 이펙트를 생성할 때 생김

**해결**: Niagara Actor를 생성한 후 Object Pooling을 통해 재활용


### 11. 메모리 최적화
**문제**: 캐릭터 용량의 1/3 ~ 1/2 정도가 스킬 텍스쳐 등 저장

**분석**: 데이터 에셋의 하드 레퍼런스로 인한 로딩

**해결**: Soft Object Reference를 사용해 필요할 때만 로딩
